 ...
 * Работа с <i>FoodShell</i>: пользователь является одним из персонажей внутренней вселенной.
 * Изначально существует только персонаж God с безумными характеристиками и локация World, являющаяся корнем мира.
 * Вселенную можно наполнять новыми локациями и новыми персонажами, персонажи взаимодействуют между собой и с локациями.
 * Все данные, характеризующие состояние мира, находятся в CSV-таблицах.
 */
public class CommandDoc {
    /**
     * Завершить работу с <i>FoodShell</i>.
     *
    ...
    /**
     * Попытка съесть пищу с заданными характеристиками.
     * Прием пищи сохраняется в таблицу CSV-объектов и дает доступ
     * к ней через команду meals.
     *
    ...
    /**
     * Просмотр списка всех существующих команд.
     *
    ...
    /**
     * Создание персонажа с заданными характеристиками или перезаписывает его, если такой персонаж есть.
     * Грусть персонажа при создании равна 0 (т.е. персонаж создается максимально счастливым).
     * Все персонажи сохраняются в соответствующую коллекцию в формате CSV.
     *
    ...
    /**
     * Вывести последовательно содержимое файлов.
     * Используется, в основном, для просмотра логов не выходя из <i>FoodShell</i>.
     *
    ...
    /**
     * Выводит список всех (нет) существующих гендеров.
     *
    ...
    /**
     * Создание новой локации или перезаписывание существующей с заданными характеристиками.
     * Все локации сохраняются в соответствующую коллекцию в формате CSV (путь к коллекции задается
     * пользователем), доступ к локациям осуществляется командой show.
     *
    ...
    /**
     * Просмотр списка имен и координат существующих локаций.
     *
     ...
     * уровень текущего насыщения, настроения, его местоположение и точные координаты.
     *
    ...
    /**
     * Залогиниться за другого пользователя-персонажа.
     *
    ...
    /**
     * Переместить текущего пользователя-персонажа в заданную локацию.
     *
     ...
     * Причина смеха не определена и остается на фантазию пользователя.
     * В результате грусть текущего пользователя уменьшается на значение силы,
     * грусть получателя же увеличивается.
     *
    ...
    /**
     * Попытка развеселить заданного персонажа на заданный уровень.
     * Если грусть персонажа-пользователя больше, чем у заданного, поднять ему настроение он не может.
     * В другом случае грусть заданного персонажа уменьшается на значение уровня.
     *
     ...
     * Причина плача не определена и остается на фантазию пользователя.
     * В результате грусть персонажа увеличивается на эту величину.
     *
    ...
    /**
     * Уничтожение локации по ее ключу. Все персонажи, которые были в этой локации, перемещаются в локацию World.
     * Локацию World уничтожить нельзя (появляется соответствующее сообщение).
     *
    ...
    /**
     * Уничтожение всех локаций (кроме World), находящихся выше чем заданная метка в мире.
     * Все персонажи из уничтоженных локаций перемещаются в локацию World.
     *
     ...
     *               со значениями чисел с плавающей запятой.
     *               Если какого-то из полей не будет, оно автоматически заполнится нулем.
     *               Остальные поля игнорируются.
     * @throws TransformerException Послано, если object не является json-объектом.
     */
    ...
    /**
     * Уничтожение всех локаций (кроме World), находящихся ниже чем заданная метка в мире.
     * Все персонажи из уничтоженных локаций перемещаются в локацию World.
     *
    ...
    /**
     * Очистка коллекции с локациями a.k.a. уничтожение всех локаций, кроме World.
     * При этом время создания локации не перезаписывается.
     *
    ...
    /**
     * Получение информации о состоянии текущей коллекции a.k.a. состоянии текущего мира.
     * Информация: дата создания мира, тип коллекции, количество локаций.
     *
